...
export const NotificationsAmount = ({ className }: { className?: string }) => {
  const { data: notifications, isLoading } = useGetAmountNotifications()

  return (
    <Link className={cn(buttonVariants({ variant: 'outline', size: 'icon' }), 'relative shrink-0', className)} href="/notifications">
